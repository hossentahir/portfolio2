...
export const Home = () => {
  ...
  const checkHealth = async () => {
    ...
    try {
      ...
      toast.success('Express server & database online!');
    } catch (err) {
      const errMsg = err.response?.data?.message || err.message || 'Server connection offline';
      setError(errMsg);
      toast.error('Backend offline. Ensure Express runs on port 5000');
    } finally {
      setLoading(false);
    }
  };
